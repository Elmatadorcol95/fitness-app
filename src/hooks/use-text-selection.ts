 ...
 * mismo índice) como a listas de campos (un índice real por fila).
 * SOLO selección — la validación/commit de cada campo sigue siendo suya.
 */
export function useIndexedTextSelection() {
  const [selections, setSelections] = useState<Record<number, TextSelectionRange | undefined>>({});

  function selectAll(index: number, text: string) {
    setSelections(prev => ({ ...prev, [index]: { start: 0, end: text.length } }));
  }

  function moveCursorToEnd(index: number, text: string) {
    setSelections(prev => ({ ...prev, [index]: { start: text.length, end: text.length } }));
  }

  return { getSelection: (index: number) => selections[index], selectAll, moveCursorToEnd };
}
